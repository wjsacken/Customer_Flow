...
HUBSPOT_CUSTOMER_TOKEN = os.getenv('HUBSPOT_CUSTOMER_TOKEN')

if not HUBSPOT_CUSTOMER_TOKEN:
    raise Exception("HUBSPOT_CUSTOMER_TOKEN environment variable is not set")

# Headers for HubSpot API requests with Bearer token
HUBSPOT_HEADERS = {
    "Authorization": f"Bearer {HUBSPOT_CUSTOMER_TOKEN}",
...
def create_or_update_contact_in_hubspot(premise, customer, sales_rep_data):
    ...
    sales_rep = None
    if pd.notna(sales_rep_id) and not sales_rep_data.empty:
    ...
    full_service_service = premise.get('services', [{}])[0].get('service_details', {}).get('full_service', {}).get('service', {})
    full_service_area = premise.get('services', [{}])[0].get('service_details', {}).get('full_service', {}).get('area', {})
    contact_properties = {
        ...
        "on_network_date_aex": format_date_to_timestamp(full_service_service.get('on_network_date', '')),
        "area_name": full_service_area.get('name', ''),
    }
    ...
    if existing_contact_id:
        # Update contact
        update_contact(existing_contact_id, {"properties": contact_properties})
        return existing_contact_id
    else:
        # Create new contact
        url = "https://api.hubapi.com/crm/v3/objects/contacts"
        response = requests.post(url, headers=HUBSPOT_HEADERS, json={"properties": contact_properties})
...
def find_existing_ticket_by_work_order_id(work_order_id):
    #Search for an existing ticket by work order ID.
    url = "https://api.hubapi.com/crm/v3/objects/tickets/search"
    search_data = {
        "filterGroups": [
            {"filters": [{"propertyName": "work_order_id1", "operator": "EQ", "value": work_order_id}]}
        ],
    }
